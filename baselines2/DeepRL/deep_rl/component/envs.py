...
def make_env(env_id, seed, rank, episode_life=True, horizon=4, dimension=30, antishaping=0.0):
    def _thunk():
        random_seed(seed)
...
class Task:
    def __init__(self,
        ...
        if log_dir is not None:
            mkdir(log_dir)
        if 'Lock-v0' in name:
            env = make_env(name, seed, 0, episode_life, horizon=horizon, dimension = dimension, antishaping = antishaping)
