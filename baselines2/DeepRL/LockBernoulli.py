...
class LockBernoulli(gym.Env):
    ...
    def init(self,env_config={}):
        ...
        self.observation_space = Box(low=0.0, high=1.0, shape=(3+self.dimension+self.horizon,),dtype=np.float)
        setattr(self.observation_space, 'n', 3+self.dimension+self.horizon)
    ...
    def make_obs(self,s):
        if self.tabular:
            ...
            new_x[s] = 1
            new_x[3:3+self.dimension] = np.random.binomial(1,0.5,(self.dimension,))
            new_x[3+self.dimension+self.h-1] = 1
            return new_x
