...
@register("cnn_maze")
def cnn_maze(**conv_kwargs):
    def network_fn(X):
        h = tf.cast(X, tf.float32) / 255.

        activ = tf.nn.relu
        h = activ(conv(h, 'c1', nf=64, rf=2, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
        h = activ(conv(h, 'c2', nf=64, rf=2, stride=2, init_scale=np.sqrt(2), **conv_kwargs))
        h = conv_to_fc(h)
        h = activ(fc(h, 'fc1', nh=128, init_scale=np.sqrt(2)))
        return h
    return network_fn
